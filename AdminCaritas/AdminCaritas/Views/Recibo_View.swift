...
struct Recibo_View: View {
    ...
    var body: some View {
        VStack{
            var listaRecibos = getRecibos(idR: recolector_id)
            Header()
            if let info_donador = listaRecibos.first(where: {$0.id == donador}){
                ...
                VStack{
                    ...
                    .popover(isPresented: $showingPopover) {
                        Cambiar_Recolector_View(donador: info_donador.id)
                            }
                }
...
struct Recibo_View_Previews: PreviewProvider {
    static var previews: some View {
        Recibo_View(donador: 1, recolector_id: 1, recolector: Repartidores(id: 1, Nombre:"Manuel", ApellidoPaterno: "Ortiz", ApellidoMaterno: "", EstadoEntrega: ""))
    }
}
